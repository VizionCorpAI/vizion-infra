# VizionAI System Wiki

## Architecture
- `architecture/00_system_overview.md`
- `architecture/01_runtime_topology.md`
- `architecture/02_execution_flow.md`
- `architecture/03_registry_and_dispatch.md`
- `architecture/04_secrets_and_identity.md`
- `architecture/05_docops_and_memory.md`
- `architecture/05_openclaw_automation.md`
- `architecture/06_security_model.md`
- `architecture/07_workspace_model.md`
- `architecture/08_maintenance_audit.md`

## Diagrams
- `diagrams/system.mmd`
- `diagrams/runtime_ports.mmd`
- `diagrams/execution_flow.mmd`
- `diagrams/registry.mmd`
- `diagrams/secrets_flow.mmd`

## Audit
- `audit/2026-02-15_audit.html`
- `audit/ports_and_services.md`
- `audit/container_inventory.md`

## Maintenance learning
- `frequent_problems/README.md`
- `how_to/README.md`
- learning plane moved to `/root/VizionAI/WORKSPACES/vizion-infra/learning/`

## Runbooks
- `runbooks/bootstrap_new_vps.md`
- `runbooks/bootstrap_new_workspace.md`
- `runbooks/rotate_secrets.md`
- `runbooks/disaster_recovery.md`
- `runbooks/container_redeployment.md`
- `runbooks/registry_recovery.md`

## Standards
- `standards/workspace_structure.md`
- `standards/naming_conventions.md`
- `standards/sql_migration_standard.md`
- `standards/agent_design_standard.md`
